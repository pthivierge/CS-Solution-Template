...
$ASM_PRODUCT="Process Data Calculation Service"

$ASM_SERVICE_DISPLAY_NAME="Process Data Calculation Service"

$ASM_SERVICE_DESCRIPTION="Service that calculates data for equipment XYZ"
...
foreach ($file in $configFiles)
{
    ...
    Foreach-Object {$_ -replace "NewApp", $SHORT_NAME -replace "%ASM_COMPANY%", $ASM_COMPANY -replace "%ASM_PRODUCT%", $ASM_PRODUCT -replace "%YEAR%", $YEAR -replace "%ASM_SERVICE_DISPLAY_NAME%",$ASM_SERVICE_DISPLAY_NAME -replace "%ASM_SERVICE_DESC%",$ASM_SERVICE_DESCRIPTION  } | 
